...
if(lc.includes('ddos-guard'))r.ddosGuard=true;if(cb.includes('recaptcha')||cb.includes('hcaptcha'))r.recaptcha=true;
...
// Phase 1: known selectors
const KNOWN=[
    '.video-item','.video-card','.video-block','.video_block','.video-thumb',
    '.thumb-item','.thumb_main','.thumb','.item','.video','.clip',
    '.gallery-item','article.post','.card','[data-video-id]','[data-id]',
    '.mozaique .thumb-block',// xvideos
    '.list-videos .video-item',// various
    'div.thumb',// tubes
];
let cards=[],uS='';
for(const s of KNOWN)try{const f=doc.querySelectorAll(s);if(f.length>=2){const hasLink=Array.from(f).some(e=>e.querySelector('a[href]'));const hasImg=Array.from(f).some(e=>e.querySelector('img'));if(hasLink&&hasImg){cards=Array.from(f);uS=s;break}}}catch{}

// Phase 2: link pattern grouping
if(!cards.length){const videoLinkPats=['/video/','/videos/','/watch/','/view/','/embed/','/v/'];
const anchors=Array.from(doc.querySelectorAll('a[href]')).filter(a=>{const h=a.getAttribute('href')||'';return videoLinkPats.some(p=>h.includes(p))||/\/\d{4,}\//.test(h)});
if(anchors.length>=3){// Find common parent
const parents=new Map();anchors.forEach(a=>{let p=a.parentElement;for(let d=0;d<5&&p;d++){const key=p.tagName+(p.className?'.'+p.className.trim().split(/\s+/)[0]:'');if(!parents.has(key))parents.set(key,{el:p,count:0,depth:d});parents.get(key).count++;p=p.parentElement}});
let bestParent=null,bestScore=0;for(const[key,data]of parents){if(data.count>=3){const score=data.count*10-data.depth;if(score>bestScore){bestScore=score;bestParent=data}}}
if(bestParent){const pp=bestParent.el;const tag=pp.tagName.toLowerCase();const cls=pp.className?'.'+pp.className.trim().split(/\s+/)[0]:'';uS=tag+cls;
// Collect siblings of same type
const container=pp.parentElement;if(container){const siblings=container.querySelectorAll(':scope > '+tag+(cls||''));if(siblings.length>=2){cards=Array.from(siblings);uS=tag+cls}else{// Use anchors' direct parents
cards=anchors.map(a=>a.closest('div,li,article,section')||a.parentElement).filter(Boolean);cards=[...new Set(cards)];uS='auto-parent'}}}}}

// Phase 3: brute force — divs/lis with img+link
if(!cards.length){const cands=[];doc.querySelectorAll('div,li,article').forEach(el=>{const imgs=el.querySelectorAll(':scope img,:scope>a>img,:scope>div>img,:scope>a>div>img');const links=el.querySelectorAll(':scope a[href]');if(imgs.length>=1&&links.length>=1&&links.length<8&&el.children.length<20){const rect=el.textContent.length;if(rect<2000)cands.push(el)}});
if(cands.length>=3){// Group by className
const groups=new Map();cands.forEach(el=>{const key=el.tagName+(el.className?'.'+el.className.trim().split(/\s+/)[0]:'');if(!groups.has(key))groups.set(key,[]);groups.get(key).push(el)});
let bestKey='',bestLen=0;for(const[k,els]of groups)if(els.length>bestLen){bestLen=els.length;bestKey=k}
...
// ---- Extract structure from first cards ----
...
if(linkEl){cd.link=resolve(linkEl.getAttribute('href'),base);
...
// Duration: scan all small elements
...
r.cardSelectors={
    container:uS,
    link:linkSel?`${uS} ${linkSel}`:null,
    title:titleSel?`${uS} ${titleSel}`:null,
    thumbnail:thumbSel?`${uS} ${thumbSel}`:null,
    thumbnailAttr:thumbAttr,
    duration:durSel?`${uS} ${durSel}`:null
};

// Fallback: derive link pattern from samples
...
// ================================================================
// MAIN PAGE PATH DETECTION
// ================================================================
function detectMainPagePath(doc,base,url){
const path=new URL(url).pathname;
const candidates=[path];
// Look for navigation links that might be "main feed"
...
// 1) Form detection
const forms=doc.querySelectorAll('form');for(const f of forms){const inputs=f.querySelectorAll('input[type="text"],input[type="search"],input[name]');for(const inp of inputs){const nm=(inp.getAttribute('name')||'').toLowerCase();if(['q','query','search','s','k','keyword','find'].includes(nm)){r.paramName=nm;r.formAction=f.getAttribute('action')||'/';r.method=(f.getAttribute('method')||'GET').toUpperCase();const action=resolve(r.formAction,base);r.pattern=action+(action.includes('?')?'&':'?')+nm+'={query}';return r}}}
// 2) JS patterns
const allJS=html+Array.from(doc.querySelectorAll('script')).map(s=>s.textContent).join('\n');
for(const pat of[/[?&](search)=[^&"']+/gi,/[?&](k)=[^&"']+/gi,/[?&](q)=[^&"']+/gi,/[?&](s)=[^&"']+/gi]){const m=pat.exec(allJS);if(m){r.paramName=m[1].toLowerCase();r.pattern=base+'/?'+r.paramName+'={query}';return r}}
// 3) Link-based
doc.querySelectorAll('a[href*="search"],a[href*="query"],a[href*="?q="],a[href*="?k="],a[href*="?s="]').forEach(a=>{if(r.paramName)return;const h=a.getAttribute('href')||'';const pm=h.match(/[?&](q|k|s|search|query)=/i);if(pm){r.paramName=pm[1].toLowerCase();r.pattern=base+'/?'+r.paramName+'={query}'}});
...
// JS NAVIGATION
// ================================================================
function parseJsNav(doc,html,base){const all=Array.from(doc.querySelectorAll('script')).map(s=>s.textContent).join('\n'),cb=all+'\n'+html;const r={categories:{fromJs:[],fromHtml:[],merged:[]},channels:{fromJs:[],fromHtml:[],merged:[],urlPattern:null},sorting:{fromJs:[]},urlScheme:{}};
// Categories
const jC=new Map();[/new\s+\w+\s*\(\s*"([^"]+)"\s*,\s*[^)]*[?&]c=([A-Za-z0-9_-]+(?:-\d+)?)/g,/new\s+\w+\s*\(\s*"([^"]+)"\s*,\s*[^)]*"[&?]c=([A-Za-z0-9_-]+(?:-\d+)?)"/g].forEach(pat=>{let m;while((m=pat.exec(cb))!==null){const n=m[1].trim(),s=m[2];if(n&&s&&!jC.has(s))jC.set(s,{name:n,slug:s,url:base+'/?c='+s})}pat.lastIndex=0});
...
for(const sel of['a[href*="/c/"]','a[href*="?c="]','a[href*="/categories/"]','a[href*="/category/"]'])try{const lnk=doc.querySelectorAll(sel);if(lnk.length>=3){lnk.forEach(a=>{const href=a.getAttribute('href'),nm=a.textContent.trim();if(href&&nm){let sl='';const cM=href.match(/[?&]c=([^&]+)/),pM=href.match(/\/c(?:ategor(?:y|ies))?\/([^/?]+)/);sl=cM?cM[1]:pM?pM[1]:href.split('/').filter(Boolean).pop()||'';r.categories.fromHtml.push({name:nm,slug:sl,url:resolve(href,base)})}});break}}catch{}
...
// Channels
const chM=new Map();[/new\s+\w+\s*\(\s*"([^"]+)"\s*,\s*[^)]*\/channels?\/([A-Za-z0-9_-]+)/g,/new\s+\w+\s*\(\s*"([^"]+)"\s*,\s*[^)]*\/pornstars?\/([A-Za-z0-9_-]+)/g].forEach(pat=>{let m;while((m=pat.exec(cb))!==null){const n=m[1].trim(),s=m[2];if(n&&s&&!chM.has(s))chM.set(s,{name:n,slug:s})}pat.lastIndex=0});
r.channels.fromJs=[...chM.values()];
for(const sel of['a[href*="/channels/"]','a[href*="/channel/"]','a[href*="/pornstar"]'])try{doc.querySelectorAll(sel).forEach(a=>{const href=a.getAttribute('href'),nm=a.textContent.trim();if(href&&nm){const sl=href.split('/').filter(Boolean).pop()||'';if(!chM.has(sl))r.channels.fromHtml.push({name:nm,slug:sl,url:resolve(href,base)})}});break}catch{}
...
doc.querySelectorAll('video source,source').forEach(s=>{const src=s.getAttribute('src')||s.getAttribute('data-src');if(!src)return;const entry={src:resolve(src,base),size:s.getAttribute('size')||null,label:s.getAttribute('label')||null,title:s.getAttribute('title')||null,dataQuality:s.getAttribute('data-quality')||null};const qa=entry.size||entry.label||entry.title||entry.dataQuality;
if(qa==='preview')return;const ql=qa?(/^\d+$/.test(qa)?qa+'p':qa):null;const method=entry.size?'size-attr':entry.label?'label-attr':entry.title?'title-attr':'unknown';if(ql&&entry.src&&!r.qualityMap[ql])r.qualityMap[ql]={url:entry.src,source:'<source>',method,domain:hostOf(entry.src)};r.sources.push(entry)})}
doc.querySelectorAll('meta[property="og:video"],meta[property="og:video:url"]').forEach(m=>{const u=m.getAttribute('content');if(u&&u.includes('.mp4')){const rv=resolve(u,base),qm=u.match(/_(\d+)\.mp4/),lb=qm?qm[1]+'p':'HD';if(!r.qualityMap[lb])r.qualityMap[lb]={url:rv,source:'og:video',method:'filename',domain:hostOf(rv)}}});
for(const cfg of JS_CFG){const found=[];for(const f of cfg.fields){const m=allJS.match(f.re);if(m){let label=f.fb;if(f.labelRe){const lm=allJS.match(f.labelRe);if(lm)label=lm[1]}const url=resolve(m[1].replace(/\\\//g,'/'),base);found.push({quality:label,url});if(!r.qualityMap[label])r.qualityMap[label]={url,source:'js-config',method:cfg.type,domain:hostOf(url)}}}if(found.length)r.jsConfigs.push({type:cfg.type,fields:found,regex:cfg.type==='kt_player'?"video_url\\s*[:=]\\s*['\"]([^'\"]+)['\"]":cfg.type==='xvideos'?"setVideoUrlHigh\\(['\"]([^'\"]+)['\"]\\)":"file\\s*:\\s*['\"]([^'\"]+)['\"]"})}
for(const vn of['dataEncodings','sources','media_sources','video_sources']){const idx=allJS.indexOf(vn);if(idx===-1)continue;const as=allJS.indexOf('[',idx);if(as===-1||as-idx>50)continue;try{let depth=0,ae=-1;for(let i=as;i<Math.min(allJS.length,as+5000);i++){if(allJS[i]==='[')depth++;else if(allJS[i]===']'){depth--;if(depth===0){ae=i;break}}}if(ae===-1)continue;JSON.parse(allJS.substring(as,ae+1)).forEach(item=>{const u=item.filename||item.file||item.src||item.url||'';const q=item.quality||item.label||item.res||item.height||'';if(!u)return;const url=(u.indexOf('//')===0?'https:':'')+u.replace(/\\\//g,'/');const key=String(q).toLowerCase()==='auto'?'auto':(q?q+'p':'auto');r.jsonEncodings.push({variable:vn,quality:key});if(!r.qualityMap[key])r.qualityMap[key]={url,source:'json-encoding',method:vn,domain:hostOf(url)}})}catch{}}
const hlsM=allJS.match(/(https?:\/\/[^\s"']+\.m3u8[^\s"']*)/);if(hlsM&&!r.qualityMap.HLS&&!r.qualityMap.auto)r.qualityMap.auto={url:hlsM[1],source:'js-regex',method:'m3u8',domain:hostOf(hlsM[1])};
...
for(const[q,info]of Object.entries(player?.qualityMap||{})){const d=info.domain;if(d&&!domains.has(d))domains.set(d,{domain:d,role:'video ('+q+')',required:true})}
...
// URL FORMAT
// ================================================================
function detectUrlFormat(html){const r={cleanUrlRules:[]};
...
const flow={catalog:{url:base,cardSelector:cards?.cardSelector,cardCount:cards?.totalCardsFound||0,linkSelector:cards?.cardSelectors?.link},card:{titleSelector:cards?.cardSelectors?.title,thumbSelector:cards?.cardSelectors?.thumbnail,thumbAttribute:cards?.cardSelectors?.thumbnailAttr,durationSelector:cards?.cardSelectors?.duration,linkPattern:cards?.linkPattern},videoPage:{strategies:[]},requiredHeaders:prot?.requiredHeaders||{}};
...
// ================================================================
// COMPATIBILITY (no duplication — only at top level)
// ================================================================
...
// ================================================================
// PARSER CONFIG — single source of truth, NO duplication
// ================================================================
...
if(cat?.navigation){const nav=cat.navigation;cfg.CATEGORIES=(nav.categories?.merged||[]).map(c=>({title:c.name,slug:c.slug}));cfg.CHANNELS=(nav.channels?.merged||[]).map(c=>({title:c.name,slug:c.slug}));cfg.SORT_OPTIONS=(nav.sorting?.fromJs||[]).map(s=>({label:s.label,value:s.param?.replace('sort=','')||null}));
cfg.URL_PATTERNS={};if(nav.urlScheme?.search)cfg.URL_PATTERNS.search=nav.urlScheme.search;if(nav.urlScheme?.category)cfg.URL_PATTERNS.category=nav.urlScheme.category;if(nav.urlScheme?.channel?.pattern)cfg.URL_PATTERNS.channel=nav.urlScheme.channel;cfg.URL_PATTERNS.pagination='&page={N}'}
...
const result={_meta:{analyzedUrl:url,baseUrl:base,analyzedAt:new Date().toISOString(),mode:'catalog',testWord:getTestWord(),tool:'v4.2.0'}};
...
const dom=aDom(doc);const fw=aFW(doc,html);
...
result.searchExamples=[{label:'Search: '+getTestWord(),url:base+'/?'+sP+'='+eTW}];result.navigation.sorting.fromJs.forEach(s=>result.searchExamples.push({label:'Search+'+s.label,url:base+'/?'+s.param+'&'+sP+'='+eTW}));
...
result._transportLog=transportLog;catalogData=result;videoPageData=null;analysisResult=buildFinalJSON();displayResults(analysisResult);setProgress(100,'✅');setStatus('✅ Каталог! Для видео → 🎬','success')}catch(e){setStatus('❌ '+e.message,'error')}finally{if(btn){btn.disabled=false;btn.textContent=isVideoMode()?'🎬 Анализ видео':'🚀 Анализ каталога'}updMerge()}}
...
const dom=aDom(doc);vd.workerWhitelist=buildWhitelist(base,dom,vd.playerStructure,catalogData?.videoCards);
setProgress(85,'🎬 Flow');
...
vd._transportLog=transportLog;videoPageData=vd;analysisResult=buildFinalJSON();displayResults(analysisResult);setProgress(100,'✅');setStatus(catalogData?'✅ Каталог + Видео = полный Config':'✅ Видео','success');
if(btn){btn.disabled=false;btn.textContent='🎬 Анализ видео'}updMerge()}

// ================================================================
// DIRECT TEST
// ================================================================
async function runDirectTest(){const url=$('targetUrl')?.value.trim();if(!url)return setStatus('❌ URL!','error');const base=baseOf(url),btn=$('btnAnalyze');if(btn){btn.disabled=true;btn.textContent='🧪'}const checks=[];setStatus('🧪','loading');setProgress(10,'🧪');
let html=null;try{const ac=new AbortController,t=setTimeout(()=>ac.abort(),12000);const r=await fetch(url,{signal:ac.signal});clearTimeout(t);if(r.ok){html=await r.text();checks.push({icon:'✅',label:'Direct fetch',hint:`HTTP ${r.status}, ${(html.length/1024|0)}KB`})}else checks.push({icon:'❌',label:'Direct fetch',hint:`HTTP ${r.status}`})}catch(e){checks.push({icon:'❌',label:'Direct fetch',hint:isCE(e)?'CORS':e.message})}
...
const testCards=aCards(doc,base);checks.push({icon:testCards.found?'✅':'❌',label:'Cards: '+(testCards.found?testCards.totalCardsFound+' ('+testCards.cardSelector+')':'not found'),hint:testCards.found?'Selector works':'Check manually'});
...
if(vUrl){setProgress(60,'🧪 Video...');try{const ac2=new AbortController,t2=setTimeout(()=>ac2.abort(),12000);const r2=await fetch(vUrl,{signal:ac2.signal});clearTimeout(t2);if(r2.ok){const vh=await r2.text();checks.push({icon:'✅',label:'Video page',hint:`${(vh.length/1024|0)}KB`});
const vDoc=parseH(vh);const vTag=vDoc.querySelector('video source[src],video[src]');const ogV=vDoc.querySelector('meta[property="og:video"]');checks.push({icon:vTag?'✅':ogV?'✅':'⚠️',label:vTag?'<video> tag':ogV?'og:video':'Video in JS only',hint:vTag?'Direct':ogV?'Meta':'Regex needed'})}}catch(e){checks.push({icon:'❌',label:'Video page',hint:e.message})}}}
...
analysisResult={_meta:{url,mode:'direct-test',tool:'v4.2.0'},directTest:{checks,verdict:verdict.v,verdictLabel:verdict.l}};
...
// ================================================================
// BUILD FINAL JSON — clean, no duplication
// ================================================================
function buildFinalJSON(){const r={_meta:{}};
if(catalogData){
r._meta={...catalogData._meta,mode:videoPageData?'catalog+video':'catalog'};
r.encoding=catalogData.encoding;
r.mainPagePaths=catalogData.mainPagePaths;
r.searchPattern=catalogData.searchPattern;
r.searchExamples=catalogData.searchExamples;
...
if(videoPageData){
if(!catalogData)r._meta={mode:'video',analyzedAt:new Date().toISOString(),tool:'v4.2.0'};
r._meta.videoPageUrl=videoPageData.url;
if(catalogData)r._meta.mode='catalog+video';
else r._meta.mode='video';
r.videoPage={url:videoPageData.url,title:videoPageData.videoTitle||videoPageData.title,poster:videoPageData.poster,player:videoPageData.playerStructure?.player,qualityMap:videoPageData.playerStructure?.qualityMap,videoUrlTemplates:videoPageData.playerStructure?.videoUrlTemplates,jsConfigs:videoPageData.playerStructure?.jsConfigs,jsonEncodings:videoPageData.playerStructure?.jsonEncodings?.length?uniq(videoPageData.playerStructure.jsonEncodings.map(e=>e.variable)):[],externalScripts:videoPageData.externalScripts?.filter(s=>s.videoFound)||[]};
r.urlFormat=videoPageData.urlFormat;
r.workerWhitelist=videoPageData.workerWhitelist;
r.parserFlow=videoPageData.parserFlow}
// Protection — from whichever has it (no dup)
const prot=catalogData?.protection||videoPageData?.protection;
if(prot){r.protection={cloudflare:prot.cloudflare,cloudflareTurnstile:prot.cloudflareTurnstile,ddosGuard:prot.ddosGuard,drm:prot.drm,drmDetails:prot.drmDetails,ageGate:prot.ageGate,refererProtected:prot.refererProtected,requiredHeaders:prot.requiredHeaders}}
// Compatibility — single instance
const jsReq=catalogData?.architecture?.jsRequired||'no';r.compatibility=assessCompat(jsReq,prot,catalogData?.videoCards,videoPageData?.playerStructure);
...
const cfg=d.parserConfig?JSON.stringify(d.parserConfig,null,2):'// No config';$('configFormatted').innerHTML=synHL(cfg);
$('btnCopyJson').disabled=false;$('btnCopyConfig').disabled=!d.parserConfig;$('btnCopyWhitelist').disabled=!d.workerWhitelist}

// ================================================================
// ARCHITECTURE RENDER
// ================================================================
function genArch(d){if(d.directTest)return'';// direct test renders itself
let h='';
...
// Whitelist
if(d.workerWhitelist?.required?.length){h+='<div class="wl-block"><h3>📡 Worker Whitelist</h3>';d.workerWhitelist.required.forEach(dm=>{h+=`<div class="wl-domain"><code>${esc(dm.domain)}</code><span class="role">${esc(dm.role)}</span></div>`});h+=`<div class="wl-code" onclick="copyWhitelist()" title="Click=copy">${esc(d.workerWhitelist.code)}</div></div>`}

// Quality Map
const qm=d.videoPage?.qualityMap;
if(qm&&Object.keys(qm).length){h+='<div class="ab"><h3 class="gt">🎬 Quality Map</h3><table class="qm-table"><tr><th>Q</th><th>URL</th><th>Source</th><th>Method</th><th>Domain</th></tr>';
for(const[q,info]of Object.entries(qm))h+=`<tr><td><strong>${esc(q)}</strong></td><td><code>${esc((info.url||'').substring(0,70))}</code></td><td>${esc(info.source)}</td><td>${esc(info.method)}</td><td><code>${esc(info.domain)}</code></td></tr>`;h+='</table></div>'}
...
h+=`<div class="pf-step"><strong>▶️ Video</strong>`;(pf.videoPage?.strategies||[]).forEach(s=>{h+=`<br>${'⭐'.repeat(s.priority)||'☆'} <code>${esc(s.method)}</code>`});h+='</div></div>';
...
// Video URL Templates
if(d.videoPage?.videoUrlTemplates?.length){h+='<div class="ab"><h3>📐 URL Templates</h3><table class="st"><tr><th>Template</th><th>Domain</th><th>Vars</th></tr>';
d.videoPage.videoUrlTemplates.forEach(t=>{h+=`<tr><td><code>${esc(t.template)}</code></td><td><code>${esc(t.domain)}</code></td><td>${(t.variables||[]).map(v=>`<span class="tag">${esc(v)}</span>`).join('')}</td></tr>`});h+='</table></div>'}
...
if(extJS?.length){h+='<div class="ab"><h3>📜 External JS</h3><div class="ext-js-block">';extJS.forEach(s=>{h+=`<div class="ext-js-item"><code>${esc(s.src)}</code><span class="ejs-st" style="color:#0f8"> ✅ video found (${((s.size/1024)|0)}K)</span></div>`});h+='</div></div>'}

// Age Gate
if(d.protection?.ageGate?.detected){const ag=d.protection.ageGate;h+=`<div class="age-g"><h4>🔞 Age Gate <span class="gt-badge ${ag.type}">${esc(ag.type)}</span></h4><p>${esc(ag.note||'')}</p>${ag.cookieName?`<div class="age-detail">Cookie: <code>${esc(ag.cookieName)}=${esc(ag.cookieValue||'1')}</code></div>`:''}</div>`}

// Recommendation
if(d.architecture?.recommendation){const rc=d.architecture.recommendation;h+=`<div class="ab"><h3 class="wt">🔧 Stack</h3><div class="arg"><span class="arl">Method:</span><span class="arv"><code>${esc(rc.method)}</code></span><span class="arl">Tools:</span><span class="arv"><code>${esc(rc.tools)}</code></span><span class="arl">Transport:</span><span class="arv">${esc(rc.transport)}</span></div></div>`}

// URL Scheme
const nav=d.navigation;if(nav?.urlScheme){h+='<div class="url-scheme"><h3>🗺️ URL Scheme</h3>';
if(d.mainPagePaths?.length){h+=`<div class="us-section"><h4>🏠 Main page</h4><div class="us-combo">`;d.mainPagePaths.forEach(p=>{h+=`<code>${esc(p)}</code> `});h+='</div></div>'}
if(d.searchExamples?.length){h+=`<div class="us-section"><h4>🔍 Search «${esc(d._meta?.testWord||'')}»</h4><table class="us-table"><tr><th>Variant</th><th>URL</th></tr>`;d.searchExamples.forEach(u=>{h+=`<tr><td style="font-size:10px">${esc(u.label)}</td><td><code>${esc(u.url)}</code></td></tr>`});h+=`</table><div class="us-combo">Pattern: <code>${esc(d.searchPattern?.pattern||nav.urlScheme.search?.pattern||'')}</code></div></div>`}
...
// Card Selectors
if(d.videoCards?.found){const cs=d.videoCards.cardSelectors;h+=`<div class="ab"><h3>🎯 Card Selectors (${d.videoCards.totalCardsFound})</h3><table class="st"><tr><th>Field</th><th>CSS Selector</th></tr>`;
...
// Sample Cards
if(d.videoCards?.sampleCards?.length){h+=`<div class="ab"><h3>📑 Sample Cards (${d.videoCards.sampleCards.length})</h3>`;
d.videoCards.sampleCards.slice(0,5).forEach((c,i)=>{h+=`<div class="sample-card"><strong>#${i+1}</strong> `;
...
// URL Format
if(d.urlFormat?.cleanUrlRules?.length){h+='<div class="ab"><h3>🔧 cleanUrl rules</h3><div class="acg">';
d.urlFormat.cleanUrlRules.forEach(r2=>{h+=`<div class="aci"><span class="aci-i">🔧</span><span class="aci-l">${esc(r2)}</span></div>`});h+='</div></div>'}
...
if(d.navigation){checks.push({i:'📁',l:'Categories',v:d.navigation.categories?.totalCount||0,c:d.navigation.categories?.totalCount?'ok':'n'});checks.push({i:'📺',l:'Channels',v:d.navigation.channels?.totalCount||0,c:d.navigation.channels?.totalCount?'ok':'n'})}
...
if(checks.length){h+='<div class="ab"><h3>✅ Summary</h3><div class="acg">';checks.forEach(c=>{h+=`<div class="aci"><span class="aci-i">${c.i}</span><span class="aci-l">${esc(c.l)}</span><span class="aci-v ${c.c}">${c.v}</span></div>`});h+='</div></div>'}
